...
function queueSongToRow(song, position, isPlaying = false) {
  return {
    room_id: ROOM_ID,
    video_id: song.id,
    title: song.title || "",
...
export default function App() {
  ...
  const loadSharedQueue = useCallback(async () => {
    ...
    const rows = data || [];
    const songs = rows.map(queueRowToSong);
    const playingIndex = rows.findIndex((row) => row.is_playing === true);

    queueRef.current = songs;
    currentIndexRef.current = playingIndex;
    setQueue(songs);
  ...
  async function addToQueue(video, playNow = false) {
    const existing = queueRef.current.some((item) => item.id === video.id);
    ...
    if (!isSupabaseConfigured) {
      ...
      const index = playNow ? next.length - 1 : currentIndexRef.current;
      ...
      if (playNow) {
        ...
        sendCommand("SYNC_QUEUE", { queue: next, currentIndex: index });
      }
    ...
    const { error } = await supabase
      .from("karaoke_queue")
      .insert(queueSongToRow(video, position, playNow));
    ...
    if (playNow) {
      ...
      const index = latest.findIndex((item) => item.id === video.id);
      if (index >= 0) {
        currentIndexRef.current = index;
        setCurrentIndex(index);
  ...
  async function handleVideoEnded() {
    ...
    await removeQueueItem(activeIndex, true);
  }
  ...
  async function playQueueIndex(index) {
    ...
    if (isSupabaseConfigured) {
      ...
      if (clearError) {
        setMessage(`Now Playing ပြောင်းမရပါ: ${clearError.message}`);
        return;
      }
      ...
      if (error) {
        setMessage(`Now Playing ပြောင်းမရပါ: ${error.message}`);
        return;
      }

      await loadSharedQueue();
      index = queueRef.current.findIndex((song) => song.dbId === selected.dbId);
    } else {
      currentIndexRef.current = index;
      setCurrentIndex(index);
    }
  ...
  async function handleNext(fromTv = false) {
    ...
    if (!fromTv && !queueRef.current.length) {
  ...
  function handlePrevious() {
    ...
    playQueueIndex(Math.max(0, currentIndexRef.current - 1));
  }

  async function removeQueueItem(index, autoAdvance = false) {
    const songs = queueRef.current;
    ...
    if (!isSupabaseConfigured) {
      ...
      if (removingCurrent) nextIndex = index < next.length ? index : -1;
      ...
      if (removingCurrent && nextIndex >= 0) {
        sendCommand("LOAD_AND_PLAY", {
          video: next[nextIndex],
          queue: next,
          index: nextIndex
        });
      } else {
        sendCommand("SYNC_QUEUE", { queue: next, currentIndex: nextIndex });
        if (removingCurrent) sendCommand("STOP");
      }
      return;
    }
    ...
    if (removingCurrent) {
      const nextSongs = queueRef.current;

      if (nextSongs.length) {
        let nextIndex = Math.min(index, nextSongs.length - 1);

        if (index >= nextSongs.length && repeatModeRef.current === "all") {
          nextIndex = 0;
        }
        ...
        setCurrentIndex(-1);
        sendCommand("SYNC_QUEUE", { queue: [], currentIndex: -1 });
        sendCommand("STOP");
  ...
  async function clearQueue() {
    ...
    sendCommand("STOP");
    setMessage("Queue ကိုရှင်းပြီး TV ကိုရပ်လိုက်ပါပြီ။");
  }
