...
export default function App() {
  ...
  function removeQueueItem(index) {
    setQueue((previous) => {
      ...
      const activeIndex = currentIndexRef.current;
      const removingCurrent = index === activeIndex;
      ...
      if (index < activeIndex) {
        ...
        nextIndex = index < next.length ? index : -1;
      }
      ...
      if (removingCurrent && nextIndex >= 0 && next[nextIndex]) {
        sendCommand("LOAD_AND_PLAY", {
  ...
  function shuffleQueue() {
    setQueue((previous) => {
      ...
      const currentId = previous[currentIndexRef.current]?.queueId;
      const next = [...previous];
      ...
      const nextIndex = currentId
        ? next.findIndex((item) => item.queueId === currentId)
        : -1;
  ...
  function reorderQueue(from, to) {
    ...
    setQueue((previous) => {
      if (!previous[from] || !previous[to]) return previous;

      const currentQueueId = previous[currentIndexRef.current]?.queueId;
      const next = [...previous];
      const [moved] = next.splice(from, 1);
      next.splice(to, 0, moved);

      const nextIndex = currentQueueId
        ? next.findIndex((item) => item.queueId === currentQueueId)
        : -1;
      ...
      <main>
        ...
        {tab === "search" && (
          <section className="panel">
            ...
            {!results.length && <div className="empty-state"><span>🎤</span><h3>သီချင်းရှာရန်အသင့်</h3><p>ရိုက်ထားတဲ့စာနောက်မှာ karaoke ထည့်ပြီး YouTube ကို တစ်ကြိမ်ပဲရှာမယ်။</p></div>}
          </section>
        )}
